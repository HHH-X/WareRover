...
class Simulator:
    ...
    def step(self):
        ...
        if clock.now() % 30 == 0:
            ...
            logger.global_logger.add_runtime_log(f"Simulator Step {clock.now()}")
        self.order_manager.step()
        idle_agv_set = self.agv_manager.get_idle_agv_ids()

        with logger.global_logger.computation_timer("scheduler"):
        ...
        with logger.global_logger.computation_timer("planner"):
