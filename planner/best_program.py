...
class FixedWindowCBSPlanner(BasePlanner):
    ...
    def _cbs_window(
        ...
        while open_list:
            ...
            for agent, constraint in zip((a1, a2), self._build_constraints(a1, a2, time, loc)):
                ...
                node_id += 1
        # Simple fallback: use individual A* without constraints
        fallback = {}
        for agv_id, (start, goal) in targets.items():
            path = self._a_star_with_constraints(
                agv_id, start, goal, carrying_status[agv_id], [], goal
    ...
    def _a_star_with_constraints(
        ...
        closed = set()
        INF = 10**9
        # Precompute goal distance for tie-breaking
        while open_heap:
    ...
    def _h(self, a, b):
        # Use Manhattan distance with a consistent tie-breaker
        dx = abs(a[0] - b[0])
        dy = abs(a[1] - b[1])
        # Add a small tie-breaker that doesn't affect admissibility (scaled by 1/1000)
        # This helps break ties and guide search more efficiently
        return dx + dy + 0.001 * (dx + dy) / (dx + dy + 1)
