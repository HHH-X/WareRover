...
class Planner:
    def __init__(self, agv_manager, map_instance, env_instance):
    ...
    def plan(self, targets: Dict[int, Tuple[Tuple[int, int], Tuple[int, int]]]) -> Dict[int, List[Tuple[int, int]]]:
        ...
        """
        # 获取当前已有路径，避免冲突
        current_paths = self.agv_manager.get_all_action_queues()
    ...
    def _a_star_with_reservation(self, start: Tuple[int, int], goal: Tuple[int, int], carrying: bool,
        ...
        while open_set:
            ...
            for neighbor in self.map.get_walkable_neighbors(current, carrying):
