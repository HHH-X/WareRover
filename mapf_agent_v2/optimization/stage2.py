...
class Stage2EvolutionRequest:
    ...
    output_root: CodeSource = "mapf_agent_v2/runs/stage2"
...
def _read_source(source: CodeSource, label: str) -> str:
    ...
    code = str(source)
    if not code.strip():
        raise ValueError(f"{label} source 为空")
    return code
...
def _build_default_config_yaml(target: OptimizationTarget) -> str:
    # No hard-coded api key. Read from env at OpenEvolve runtime.
    return textwrap.dedent(
        ...
        prompt:
          system_message: "Optimize MAPF {target.value} implementation while keeping interface compatibility."
        ...
        def evaluate(program_path: str):
            try:
                ...
                if TARGET in ("planner", "both"):
                    PlannerRegistry.register(PLANNER_REG_NAME, _pick_subclass(module, BasePlanner, "planner"))
                if TARGET in ("scheduler", "both"):
                    SchedulerRegistry.register(SCHEDULER_REG_NAME, _pick_subclass(module, BaseScheduler, "scheduler"))
                run_metrics = []
                for seed in SEEDS:
                    if TARGET == "planner":
                        active_planner, active_scheduler = PLANNER_REG_NAME, BASELINE_SCHEDULER
                    elif TARGET == "scheduler":
                        active_planner, active_scheduler = BASELINE_PLANNER, SCHEDULER_REG_NAME
                    else:
                        active_planner, active_scheduler = PLANNER_REG_NAME, SCHEDULER_REG_NAME
                ...
                combined_score = 0.35 * completion_score + 0.35 * makespan_score + 0.20 * time_score + 0.10 * stability_score
                return {{
...
def run_stage2_evolution(request: Stage2EvolutionRequest) -> Stage2EvolutionResult:
    ...
    output_dir = run_dir / "openevolve_output"

    initial_program_path.write_text(_build_initial_program(target, planner_code, scheduler_code), encoding="utf-8")
    evaluator_path.write_text(_build_evaluator_code(request, target), encoding="utf-8")
    config_path.write_text(
        _read_source(request.config_path, "config") if request.config_path else _build_default_config_yaml(target),
        encoding="utf-8",
    )
